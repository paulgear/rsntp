...
mod tests {
    ...
    #[test]
    fn test_update_first_seen_time_no_recursion() {
        let collector = MetricsCollector::new(&[(100, 60), (1000, 3600), (10000, 86400)]);
        println!("About to call update_first_seen_time with thread_id 0");
        collector.update_first_seen_time(PacketEvent::ServerRequestReceived, 0);
        println!("Done running update_first_seen_time with thread_id 0");
        // Test passes if no panic occurs
    }

    #[test]
    fn test_gauge_get_after_registry() {
        let collector = MetricsCollector::new(&[(100, 60), (1000, 3600), (10000, 86400)]);
        let labels = PacketLabels {
            thread_id: "1".to_string(),
            packet_event: "test".to_string(),
        };
        println!("Getting first gauge");
        let gauge1 = collector.first_seen_gauge.get_or_create(&labels);
        println!("Got first gauge, calling get()");
        let val1 = gauge1.get();
        println!("First gauge value: {}", val1);

        let labels2 = PacketLabels {
            thread_id: "0".to_string(),
            packet_event: "test".to_string(),
        };
        println!("Getting second gauge");
        let gauge2 = collector.first_seen_gauge.get_or_create(&labels2);
        println!("Got second gauge, calling get()");
        let val2 = gauge2.get();
        println!("Second gauge value: {}", val2);
    }

    #[test]
    fn test_gauge_without_registry() {
        use prometheus_client::metrics::family::Family;
        use prometheus_client::metrics::gauge::Gauge;

        let family: Family<PacketLabels, Gauge> = Family::default();
        let labels1 = PacketLabels {
            thread_id: "1".to_string(),
            packet_event: "test".to_string(),
        };
        println!("Getting first gauge (no registry)");
        let gauge1 = family.get_or_create(&labels1);
        println!("Got first gauge, calling get()");
        let val1 = gauge1.get();
        println!("First gauge value: {}", val1);
        ...
        println!("Getting second gauge (no registry)");
        let gauge2 = family.get_or_create(&labels2);
        println!("Got second gauge, calling get()");
        let val2 = gauge2.get();
        println!("Second gauge value: {}", val2);
    }
